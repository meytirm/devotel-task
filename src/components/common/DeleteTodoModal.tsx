    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Remove Todo">
      <div>
        ...
        <div className="flex justify-end gap-4 mt-4">
          <Button onClick={handleRemoveTodo} className="bg-red-500 hover:bg-red-600 text-white" disabled={remove}>
            {remove ? <LoaderCircleIcon className="animate-spin"/> : 'Remove'}
          </Button>
          <Button onClick={() => onClose(false)} className="bg-gray-300 hover:bg-gray-300/80" disabled={remove}>
            Cancel
          </Button>
        </div>
      </div>
